...
func OnSendPaging(ctx sbi.RequestContext, prod Producer) {
	...
	prob := prod.HandleSendPaging(ctx.Context(), body)
...
func OnAmfSubscribe(ctx sbi.RequestContext, prod Producer) {
	...
	// call application handler
	rsp, prob := prod.HandleAmfSubscribe(ctx.Context(), callback, body)
...
type Producer interface {
	HandleSendPaging(context.Context, *models.PagingMessage) *models.ProblemDetails

	HandleAmfSubscribe(context.Context, *models.EndpointInfo, *models.AmfSubscribeRequest) (*models.AmfSubscribeResponse, *models.ProblemDetails)
}
